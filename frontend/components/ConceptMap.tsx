...
export function ConceptMap() {
  const { topic, keyphrases, sources, selectedKeyphrase, selectKeyphrase, setTopic, generate } =
    useAppStore();
  ...
  const { nodes: initialNodes, edges: initialEdges } = useMemo(
    ...
    [topic, keyphrases, sources, selectedKeyphrase, handleSelect, handleExplore]
  );
  ...
  return (
    <div className="h-[340px] w-full overflow-hidden rounded-lg border bg-white">
      <ReactFlow
        key={flowKey}
        nodes={initialNodes}
        edges={initialEdges}
